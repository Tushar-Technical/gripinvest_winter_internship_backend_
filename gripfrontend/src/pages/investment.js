...
export default function Investment({ setUser }) {
...
const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      let res= await axios.post("http://0.0.0.0:3001/investment/products",{ ...form ,token:localStorage.getItem("token")});
      // console.log(res)
      if(res.data.admin){
  ...
  return (
     ...
     <div className="form-container">
      ...
      <form className="modern-form" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn-submit">List Product</button>
        <br></br>
